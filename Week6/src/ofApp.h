...
public:
	...
	float wallBreaktimer;
	float wallBreaktimerDuration;
	
	
	void initializeRandomWorld ();
	void generateMap();
	void paintMap ();
	int countAliveNeighbors(vector<basicSprite *> _backgrounds, int x, int y);
	...
	basicSprite* getTile(int x, int y);
